...
class CSMN(object):
  ...
  def __init__(self, inputs, config, name="CSMN", is_training=True):
    ...
    def _loop_body(iterator_, output_words_array_,
      ...
      #                              alpha_m_re, tf.stack(tf.unstack(context, axis=1),axis=0),
      #                          ),
      #                          axis=1
      #                        )# ,
                      # [self.batch_size, -1]
                      # )
      # alpha_numerator = tf.exp(alpha_numerator)
                      
      # def cond(i, alpha_numerator): 
      #     return i < tf.to_int32(self.max_context_length)
      # def body(i, alpha_numerator): 
      #     return i+1, alpha_numerator + tf.exp(
      #                   tf.reduce_sum(
      #                       tf.nn.embedding_lookup(
      #                           alpha_m_re, tf.gather(tf.unstack(context, axis=1), tf.to_int32(i)))))
      # i, alpha_numerator = tf.while_loop(
      #         cond, body, (0, 0))
